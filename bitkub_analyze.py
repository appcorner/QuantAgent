#!/usr/bin/env python3
"""
Bitkub Analyze — CLI command for trading analysis using OHLC data from Bitkub Exchange.

This script mirrors the behaviour of the /api/analyze web endpoint but fetches
data from Bitkub's public TradingView history API instead of Yahoo Finance.

Usage examples:
    # Fetch the latest 100 bars on H1
    python bitkub_analyze.py --symbol BTC_THB --timeframe 1h --bars 100

    # Fetch by date range
    python bitkub_analyze.py --symbol ETH_THB --timeframe 4h \\
        --start "2025-01-01" --end "2025-01-31"

    # Export results to JSON
    python bitkub_analyze.py --symbol BTC_THB --timeframe 1d --bars 50 \\
        --output result.json
"""
...
from bitkub_data import BitkubClient  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run QuantAgent trading analysis on OHLC data from Bitkub Exchange.",
    ...
    parser.add_argument(
        "--symbol",
        required=True,
        help="Bitkub symbol, e.g. BTC_THB, ETH_THB, DOT_THB",
    )
    parser.add_argument(
        "--timeframe",
        default="1h",
        choices=list(BitkubClient.TIMEFRAME_MAP.keys()),
...
def main() -> None:
    ...
    # ── 1. Check Bitkub status ────────────────────────────────────────
    client = BitkubClient()

    print(f"{CYAN}Checking Bitkub API status …{RESET}")
    status = client.check_status()
    if status.get("status") == "error":
        print(f"{RED}❌ Cannot reach Bitkub API: {status.get('detail')}{RESET}")
        sys.exit(1)
    print(f"{GREEN}✅ Bitkub API is {status.get('status', 'unknown')}{RESET}")
