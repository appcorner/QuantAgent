...
1. Favor **actionable** signals: if at least **2 of 3 reports** (Indicator, Pattern, Trend) align with clear direction, treat it as tradable unless there is a strong contradiction.
2. **Do not require perfect alignment of all three reports**. When one report is neutral but not conflicting, follow the side supported by momentum and price structure.
...
- Set **should_enter_now = true** by default when directional bias is clear and there is no major execution handicap.
- A single valid trigger is enough (no need for multiple simultaneous confirmations), for example:
- Breakout/breakdown with acceptable momentum,
- Clean bounce/rejection at support-resistance,
- Trend continuation after a shallow pullback or retest.
- Set **should_enter_now = false** only when risk is clearly unfavorable, such as:
- Price is severely overextended after an abnormal impulse candle,
- Entry is immediately blocked by very nearby strong support/resistance with poor payoff,
- Structure is highly noisy and direction is genuinely ambiguous.
