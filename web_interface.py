...
@app.route("/api/analyze-bitkub", methods=["POST"])
def analyze_bitkub():
    """Run trading analysis using OHLC data from Bitkub Exchange.

    Request JSON body:
        symbol      (str, required): Bitkub symbol, e.g. BTC_THB, ETH_THB
        timeframe   (str, optional): 1m|5m|15m|1h|4h|1d  (default: 1h)
        bars        (int, optional): Number of latest bars to fetch (default: 100)
    ...
    try:
        ...
        if not symbol:
            return jsonify({"error": "Symbol is required (e.g. BTC_THB)."})

        # Validate timeframe
        if timeframe not in BitkubClient.TIMEFRAME_MAP:
            return jsonify(
                {"error": f"Unsupported timeframe '{timeframe}'. "
                          f"Choose from: {', '.join(BitkubClient.TIMEFRAME_MAP.keys())}"}
            )
...
@app.route("/api/bitkub-symbols")
def bitkub_symbols():
    """List all available Bitkub trading symbols."""
    try:
        client = BitkubClient()
